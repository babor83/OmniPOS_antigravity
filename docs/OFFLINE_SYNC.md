...
This document describes the offline invoice synchronization system in omniPOS, including the deduplication mechanism that prevents duplicate invoices from being created during network failures.

## Overview

omniPOS supports fully offline operation, allowing cashiers to continue creating invoices even when the network connection is unavailable. When connectivity is restored, pending invoices are automatically synchronized to the server.
...
omnipos/omnipos/doctype/offline_invoice_sync/
...
```python
# Endpoint: /api/method/omnipos.api.invoices.check_offline_invoice_synced
# Method: POST
...
```python
# Endpoint: /api/method/omnipos.api.invoices.submit_invoice
# Method: POST
# Params:
