"""
Uninstallation hooks for omniPOS
"""
import frappe
import logging
...
def before_uninstall():
	...
	"""
	try:
		log_message("Starting omniPOS uninstallation", level="info")
		...
		log_message("omniPOS uninstalled successfully", level="success")
		log_message("All custom fields and configurations have been removed", level="info")

	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(
			title="omniPOS Uninstallation Error",
			message=frappe.get_traceback()
		)
		log_message(f"Error during omniPOS uninstallation: {str(e)}", level="error")
		raise


def remove_custom_fields():
	"""
	Remove all custom fields created by omniPOS
	"""
	try:
...
def remove_print_formats():
	"""
	Remove all print formats created by omniPOS
	"""
	try:
...
def reset_pos_profiles():
	"""
	Reset POS Profile configurations set by omniPOS
	"""
	try:
		log_message("Resetting POS Profile configurations", level="info")

		# Find POS Profiles using omniPOS print format
		pos_profiles = frappe.get_all(
			"POS Profile",
			filters={"print_format": "omniPOS Receipt"},
			fields=["name"]
		)

		if not pos_profiles:
			log_message("No POS Profiles using omniPOS configurations", level="info", indent=1)
			return
...
def get_custom_fields_for_cleanup():
	...
	Returns list of field names that belong to omniPOS
	"""
	custom_fields = []

	# Always safe to remove (omniPOS specific)
