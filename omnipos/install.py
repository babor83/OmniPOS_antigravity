"""
Installation and Migration hooks for omniPOS
...
def after_install():
	...
	try:
		log_message("omniPOS: Running post-install setup", level="info")
		...
		log_message("omniPOS: Installation completed successfully", level="success")
	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(
			title="omniPOS Installation Error",
			message=frappe.get_traceback()
		)
		log_message(f"omniPOS: Installation error - {str(e)}", level="error")
		raise
...
def after_migrate():
	...
	try:
		...
		log_message("omniPOS: Migration completed successfully", level="success")
	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(
			title="omniPOS Migration Error",
			message=frappe.get_traceback()
		)
		log_message(f"omniPOS: Migration error - {str(e)}", level="error")
		raise


def setup_default_print_format(quiet=False):
	"""
	Set omniPOS Receipt as default print format for POS Profiles if not already set.
	...
	try:
		...
		if not frappe.db.exists("Print Format", "omniPOS Receipt"):
			if not quiet:
				log_message("omniPOS Receipt print format not found, skipping default setup", level="warning")
			return
