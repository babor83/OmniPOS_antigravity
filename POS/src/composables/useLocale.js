...
const PREFARED_LANGUAGE_KEY = "omnipos_language"
const ALLOWED_LOCALES_KEY = "omnipos_allowed_locales"
...
async function fetchAllowedLocalesFromServer() {
	try {
		const response = await call("omnipos.api.localization.get_allowed_locales", {})
...
async function fetchLanguageFromServer() {
	...
	try {
		const response = await call("omnipos.api.localization.get_user_language", {})
...
export function useLocale() {
	...
	async function changeLocale(newLocale) {
		...
		try {
			await call("omnipos.api.localization.change_user_language", {
				locale: newLocale,
			})
		} catch (error) {
