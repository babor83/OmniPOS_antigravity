...
const paymentMethodsResource = createResource({
	url: "omnipos.api.pos_profile.get_payment_methods",
	makeParams() {
		return {
...
const customerCreditResource = createResource({
	url: "omnipos.api.credit_sales.get_available_credit",
	makeParams() {
...
const customerBalanceResource = createResource({
	url: "omnipos.api.credit_sales.get_customer_balance",
	makeParams() {
		const customerName = props.customer?.name || props.customer
...
const walletInfoResource = createResource({
	url: "omnipos.api.wallet.get_wallet_info",
	makeParams() {
		const customerName = props.customer?.name || props.customer
		log.debug(
...
async function identifyWalletPaymentMethods() {
	...
	try {
		...
		const result = await call(
			"omnipos.api.pos_profile.get_wallet_payment_flags",
			{
				methods: methodNames,
			},
		)
...
const salesPersonsResource = createResource({
	url: "omnipos.api.pos_profile.get_sales_persons",
	makeParams() {
		return {
			pos_profile: props.posProfile,
		}
