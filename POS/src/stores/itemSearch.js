...
async function cacheVariantsForTemplates(items, posProfile) {
	...
	for (let i = 0; i < templateItems.length; i += CONCURRENCY_LIMIT) {
		...
		const batchPromises = batch.map(async (template) => {
			try {
				const response = await call("omnipos.api.items.get_item_variants", {
...
async function cacheBatchSerialForItems(items, warehouse) {
	...
	for (let i = 0; i < itemCodes.length; i += BATCH_SIZE) {
		...
		try {
			const response = await call("omnipos.api.items.get_batch_serial_data_for_items", {
...
export const useItemSearchStore = defineStore("itemSearch", () => {
	...
	const itemGroupsResource = createResource({
		url: "omnipos.api.items.get_item_groups",
		makeParams() {
			return {
	...
	async function loadAllItems(profile, forceServerFetch = false) {
		...
		try {
			...
			const countPromise = !offline ? call("omnipos.api.items.get_items_count", {
				pos_profile: profile,
			...
			// large catalogs (1000+ items) to minimize initial load time.
				...
				const response = await call("omnipos.api.items.get_items", {
					pos_profile: profile,
					search_term: "",
	...
	async function fetchItemsForGroup(profile, itemGroup, start = 0, limit = null) {
		...
		try {
			...
			if (groupsToFetch.length > 1) {
				const response = await call("omnipos.api.items.get_items_bulk", {
					pos_profile: profile,
	...
	async function startBackgroundCacheSync(profile, filterGroups = [], initialOffset = 0) {
		...
		try {
			...
			const countResponse = await call("omnipos.api.items.get_items_count", {
				pos_profile: profile,
				include_variants: 1,
			})
			...
			while (myGeneration === syncGeneration) {
				try {
					...
					if (hasFilters && groupsToSync.length > 0) {
						...
						response = await call("omnipos.api.items.get_items", {
							pos_profile: profile,
							search_term: "",
	...
	async function setSelectedItemGroup(group) {
		...
		if (posProfile.value) {
			...
			try {
				...
				const countPromise = call("omnipos.api.items.get_items_count", {
					pos_profile: posProfile.value,
	...
	async function setPosProfile(profile, autoLoadItems = true) {
		...
		try {
			...
			const data = await call("omnipos.api.pos_profile.get_pos_profile_data", {
				pos_profile: profile
			})
