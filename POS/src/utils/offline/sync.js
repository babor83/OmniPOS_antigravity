...
export const pingServer = async () => {
	...
	try {
		...
		const response = await fetch("/api/method/omnipos.api.ping", {
			method: "GET",
...
export const checkOfflineIdSynced = async (offlineId) => {
	...
	try {
		const response = await call(
			"omnipos.api.invoices.check_offline_invoice_synced",
			{ offline_id: offlineId },
		)
...
const syncInvoiceToServer = async (invoice, retryCount = 0) => {
	...
	try {
		const response = await call("omnipos.api.invoices.submit_invoice", {
