...
function getCacheVersion() {
	...
	const storedHash = localStorage.getItem("omnipos_cache_structure_hash")
	const storedVersion = Number.parseInt(
		localStorage.getItem("omnipos_cache_version") || "1",
	)
	...
	if (storedHash !== structureHash.toString()) {
		...
		localStorage.setItem(
			"omnipos_cache_structure_hash",
			structureHash.toString(),
		)
		localStorage.setItem("omnipos_cache_version", newVersion.toString())
		return newVersion
	}
...
export const cacheCustomersFromServer = async (posProfile) => {
	try {
		...
		const response = await call("omnipos.api.customers.get_customers", {
                        pos_profile: posProfile,
                        start: 0,
...
export async function cachePaymentMethodsFromServer(posProfile) {
	try {
		const result = await call("omnipos.api.pos_profile.get_payment_methods", {
			pos_profile: posProfile,
		})
