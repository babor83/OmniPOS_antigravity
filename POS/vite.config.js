...
function posNextBuildVersionPlugin(version) {
	return {
		...
		apply: "build",
		async writeBundle() {
			const versionFile = path.resolve(__dirname, "../omnipos/public/pos/version.json")
...
export default defineConfig({
	plugins: [
		...
		frappeui({
			...
			lucideIcons: true,
			buildConfig: {
				indexHtmlPath: "../omnipos/www/pos.html",
				outDir: "../omnipos/public/pos",
				emptyOutDir: true,
		...
		VitePWA({
			...
			manifest: {
				...
				display: "standalone",
				scope: "/assets/omnipos/pos/",
				start_url: "/pos",
				icons: [
					{
						src: "/assets/omnipos/pos/icon.svg",
						sizes: "192x192",
						type: "image/svg+xml",
						purpose: "any",
					},
					...
					{
						src: "/assets/omnipos/pos/icon-maskable.svg",
						sizes: "192x192",
						type: "image/svg+xml",
						purpose: "maskable",
					},
			...
			workbox: {
				...
				runtimeCaching: [
					...
					{
						urlPattern: /\/assets\/omnipos\/pos\/.*/i,
						handler: "CacheFirst",
						options: {
